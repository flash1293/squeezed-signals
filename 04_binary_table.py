#!/usr/bin/env python3
"""
Phase 4: Binary Table + zstd - String Deduplication and Compression

This script stores data in a binary table format that eliminates key repetition
with zstd compression applied. This combines structural optimization with 
general-purpose compression for better results.
"""
...
def main():
    ...
    print("=" * 60)
    print("Phase 4: Binary Table + zstd")
    print("=" * 60)
    ...
    output_file = "output/metrics.bintable.zst"
    with open(output_file, "wb") as f:
        f.write(compressed_data)

    file_size = len(compressed_data)
    compression_ratio = uncompressed_size / file_size    # Verify by decompressing and decoding
    print("\nVerifying compressed binary table format...")
    decompressor = zstd.ZstdDecompressor()
    decompressed_data = decompressor.decompress(compressed_data)
    decoded_points = decode_binary_table_format(decompressed_data)
    ...
    cbor_zst_file = "output/metrics.cbor.zst"
    
    ndjson_size = os.path.getsize(ndjson_file) if os.path.exists(ndjson_file) else None
    cbor_zst_size = os.path.getsize(cbor_zst_file) if os.path.exists(cbor_zst_file) else None
    ...
    if cbor_zst_size:
        vs_cbor_zst = cbor_zst_size / file_size
        print(f"\n📉 Comparison vs CBOR + zstd:")
        print(f"  CBOR + zstd size: {cbor_zst_size:,} bytes")
        print(f"  Binary table + zstd size: {file_size:,} bytes")
...
if __name__ == "__main__":
    ...
    if result:
        print(f"\n✅ Phase 4 completed in {end_time - start_time:.2f} seconds")
    else:
        print(f"\n❌ Phase 4 failed after {end_time - start_time:.2f} seconds")