...
def main():
    ...
    output_file = "output/metrics.columnar.msgpack"
    file_size = store_as_columnar(columnar_data, output_file)
    
    # Compare with NDJSON baseline
    ndjson_file = "output/metrics.ndjson"
    ndjson_size = os.path.getsize(ndjson_file) if os.path.exists(ndjson_file) else None
    
    print(f"\n📊 Columnar Storage Results:")
    ...
    if ndjson_size:
        ...
        print(f"  Columnar size: {file_size:,} bytes")
        print(f"  Compression ratio: {compression_ratio:.2f}x")
        print(f"  Space saved: {space_saved:,} bytes ({space_saved / ndjson_size * 100:.1f}%)")
    
    print(f"\n💡 Columnar Format Characteristics:")
    print(f"  ✅ Pros:")
    print(f"    - Eliminates key repetition")
    print(f"    - Groups related data together")
    print(f"    - Enables column-specific compression")
    print(f"    - Faster queries for single series")
    print(f"    - Binary format is more space-efficient")
    print(f"  ❌ Cons:")
